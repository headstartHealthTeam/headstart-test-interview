...
export class InterviewService {
  ...
  async submitResponse(candidateId: number, submitResponseDto: SubmitResponseDto): Promise<Response> {
  ...
  async getCandidateResults(candidateId: number) {
    ...
    const totalPointsEarned = candidate.responses.reduce((sum, response) => sum + response.pointsEarned, 0);
    const correctAnswers = candidate.responses.filter(response => response.isCorrect).length;
  ...
  async seedQuestions() {
    const questions = [
      ...
      },
      {
        ...
        correctAnswer: 3,
        explanation: 'TypeScript uses "Array<T>" or "T[]" for array types, not "array".',
        points: 1
      },
      {
      ...
      },
      {
        ...
        correctAnswer: 1,
        explanation: 'POST is typically used for creating new resources in REST APIs.',
        points: 1
      },
      {
        ...
        correctAnswer: 1,
        explanation: 'TypeORM is an ORM (Object-Relational Mapping) library for TypeScript and JavaScript.',
        points: 1
      }
    ];
